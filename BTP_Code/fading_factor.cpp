...
void create(string lbl,int time,int seq,node *par,double frequency)
{
node *t=new node();
t->left=0;
t->next=0;
...
create(store[i],curr_time,seq_id,root,1);
//}
}
}
...
void g(node *curr,int curr_time,string items[],int seq,int poi,double threshold)
{
if(curr->freq<threshold)
{
...
//if(tmp==0)
/********************/
create(store[i],curr_time,curr->seq_id,curr,curr->freq);
//else
//tmp->ts=curr_time;
}
}

void main_func(node *curr,int curr_time,string items[],int seq,int poi,double threshold)
{
if(curr==root)
f(curr_time,items,seq,poi);
else
g(curr,curr_time,items,seq,poi,threshold);
}

void traverse(node *curr,int curr_time,string items[],int seq,int poi,double lambda,double threshold)
{
if(curr==0)
return;
node *mv=curr->left;
while(mv!=0)
{
traverse(mv,curr_time,items,seq,poi,lambda,threshold);
mv=mv->next;
}
curr->freq=(curr->freq)*lambda;
main_func(curr,curr_time,items,seq,poi,threshold);
}
...
double d_factor_inverse(double frequency,int d_factor)
{
...
double min_support=0.5,lambda,threshold,count_pat;
...
for(min_support=0.09;min_support<=0.091;min_support+=0.02)
{
count_pat=0;
...
{
p1.traverse(r,curr_time,items[curr_time-1],seq,poi,lambda,threshold);
...
vector<pair<string,double> > freq_pattern;

for(map<string,double>::iterator it=(p1.frequency).begin();it!=(p1.frequency).end();it++)
{
/*if(curr_time==2)
{
...
//cout<<(*it).first<<" "<<(*it).second<<" "<<((double)((*it).second))/seq<<endl;
...
//count_pat=0;
//uncommentcout<<endl<<endl<<"Frequent Patterns for Timestamp t = "<<curr_time<<" are :"<<endl<<endl;

for(int i=0;i<freq_pattern.size();i++)
{
if(freq_pattern[i].first.length()>=min_len)
{
/*uncoomentfor(int h=0;h<(freq_pattern[i].first).length();h++)
...
count_pat+=d_factor_inverse(freq_pattern[i].second,d_factor);
